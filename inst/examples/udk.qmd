...
In total, there are `r length(df.orig$UDC)` entries in Fennica, with `r sum(is.na(df.orig$UDC) | df.orig$UDC == "")` missing UDC values in our dataset.
...
[List of Unrecognized UDC with count ](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/UDK_discarded.csv): `r length(unique(df.tmp1$udk))`

[Unrecognized UDC with Melinda ID: Error list](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/udk_errors_list.csv.csv)
...
UDC codes are from [the National Library of Finland](https://www.kiwi.fi/spaces/Kansallisbibliografiapalvelut/pages/57836458/Monografia-aineiston%2BUDK-luokituskaavio); new custom abbreviations can be added in [this table](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/udk.csv).
...
There are `r sum(df_19$primary == "Undetermined", na.rm = TRUE)` entries where the primary UDC is "Undetermined" (`r round(mean(df_19$primary == "Undetermined", na.rm = TRUE) * 100)`%)
