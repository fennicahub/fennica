...
This plot visualizes the variation in title lengths across publication decades from 1488 to 2020. The title lengths range from  `r min(df$title_length)` to`r max(df$title_length)` , highlighting how the length of titles has evolved over time.N = `r nrow(df)`.
...
This plot visualizes the variation in title lengths across publication years from 1809 to 1917. The title lengths range from  `r min(df_19$title_length)` to`r max(df_19$title_length)`. N = `r nrow(df_19)`.
