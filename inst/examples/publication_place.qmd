...
MARC: [260a](https://www.loc.gov/marc/bibliographic/bd260.html) and [264a](https://www.loc.gov/marc/bibliographic/bd264.html)
...
-   [Ambiguous publication places](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/publication_place_ambiguous.csv);

-   [Unknown place names](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/publication_place_todo.csv) 

Terms that are clearly not place names can be added to [stopwords](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/stopwords_for_place.csv);

- [Discarded place names](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/publication_place_discarded.csv) 
...
p <- p + ggtitle(paste("Top publication places (log10)"))
p <- p + scale_y_log10()
p <- p + ylab("Title count")
print(p)

p <- top_plot(df_19, "publication_country", ntop)
p <- p + ggtitle(paste("Top publication countries (log10)"))
p <- p + scale_y_log10()
p <- p + ylab("Title count")
print(p)
```
...
```{r}
#| label = "publication-country-world-map",
#| echo = FALSE,
#| message = FALSE,
#| warning = FALSE,
#| fig.width = 12,
...
country_counts <- df %>%
  filter(!is.na(publication_country), str_trim(publication_country) != "") %>%
...
p <- ggplot(world_map_df) +
  ...
  scale_fill_viridis_c(
    ...
    na.value = "grey95",
    labels = comma,
    name = "Documents (log scale)"
  ) +
  labs(
    title = "Publication countries in the complete dataset"
  ) +
...
```{r}
#| label = "publication-finland-map-19",
#| echo = FALSE,
#| message = FALSE,
#| warning = FALSE,
#| fig.width = 15,
...
# Aggregate publication places in the 1809–1917 subset
finland_places_19 <- df_19 %>%
  filter(
  ...
  mutate(
    publication_place = str_trim(publication_place)
  ) %>%
  count(publication_place, latitude, longitude, name = "n") %>%
  arrange(desc(n))

# Finland base map
finland <- rnaturalearth::ne_countries(
  country = "Finland",
  scale = "medium",
  returnclass = "sf"
)
...
h <- ggplot() +
  ...
  labs(
    title = "Publishing in Finland, 1809–1917",
  ) +
  theme_minimal(base_size = 12) +
  theme(
