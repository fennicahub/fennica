...
The field has `r sum(!is.na(df[[field]]))` / (`r round(100*mean(!is.na(df[[field]])), 1)`%) non-missing values and  `r sum(is.na(df[[field]]))` / (`r round(100*mean(is.na(df[[field]])), 1)`%) with missing lifetime information. There are `r sum(is.na(df.orig[[field]]))` records with missing author name information in the original raw data.
