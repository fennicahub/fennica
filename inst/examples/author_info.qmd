...
MARC21: [100a](https://www.loc.gov/marc/bibliographic/bd100.html) and [700a](https://www.loc.gov/marc/bibliographic/bd700.html)
[KANTO database](https://finto.fi/finaf/en/)
...
In total there are XXX unique auhtors in fennica (asteri + name + date, NA + name + date, asteri + name + name). so many auhtrors have id and so many dont have them. in total there are so many records wihtout a name. gender is assigned to auhtors who created until 1917. auhtor birth date is avaiable for som many, auhtor date is available for so many. both for so many and consequnrtly age is available for the same amount. others with the most records in fennica are here. profession is availble for this many people and the most poular profession on fennica are. discraded rcords are show for each separate fields tgether with error lists. we have two databases record based and id and asteri based it. 

datatables: unique names with their dates, with gender, number of records, profession, other infp
...
```{r}
#| include = FALSE
source("author_all_names.R")
```
-   [Unique accepted entries in original data](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/author_name_enriched_accepted.csv): `r length(unique(df[[field]]))`
-   [Unique discarded entries in original data](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/author_name_enriched_discarded.csv) (excluding NA cases): `r as.vector(countLines(file_discarded) - 1)`
-   Original documents with non-NA titles `r sum(!is.na(df[[field]]))` / `r nrow(df)` (`r round(100*mean(!is.na(df[[field]])), 1)`%)
-   Original documents with missing (NA) titles `r sum(is.na(df[[field]]))` / `r nrow(df)` documents (`r round(100*mean(is.na(df[[field]])), 1)`%)

# Author's Name after Kanto integration

```{r}
#| label = "summarytitle-author-all-names",
#| echo = FALSE,
#| message = FALSE,
#| warning = FALSE,
#| fig.width = 8,
...
# 1809-1917 all names
```{r}
#| label = "summarytitle-author-all-names19",
#| echo = FALSE,
#| message = FALSE,
#| warning = FALSE,
#| fig.width = 8,
...
ggplot(avg_age_by_year, aes(x = first_pub_year, y = mean_age)) +
  ...
  labs(
    ...
    title = "Smoothed Trend in Average Age at First Publication (1800–Present)"
  ) +
