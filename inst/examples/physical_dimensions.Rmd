...
## Document size comparisons
  ...
  * Some dimension info is provided in the original raw data for altogether `r sum(!is.na(df$physical_dimensions))` documents (`r round(100*mean(!is.na(df$physical_dimensions)),1)`%) but could not be interpreted for `r sum(!is.na(df$physical_dimensions) & (is.na(df$gatherings)))` documents (ie. dimension info was successfully estimated for `r round(100 - 100 * sum(!is.na(df$physical_dimensions) & (is.na(df$gatherings)))/sum(!is.na(df$physical_dimensions)), 1)` % of the documents where this field was not empty).
...
nmin <- 2000
top.gatherings <- setdiff(names(which(table(df$gatheringsinal) > nmin)), "NA")

df2 <- filter(df, !gatheringsinal == "NA" &
                  (!is.na(heightinal) | !is.na(widthinal))) %>%
       filter(gatheringsinal %in% top.gatherings) %>%
       select(publication_decade, gatheringsinal, heightinal, widthinal)
       
df3 <- df2 %>% group_by(gatheringsinal, publication_decade) %>% 
       summarize(mean.heightinal = mean(heightinal, na.rm = T),
    	         mean.widthinal  = mean(widthinal, na.rm = T),
		 n = n())
...
p <- p + geom_point(data = df3, aes(x = publication_decade,
       	 		   	    y = mean.heightinal,
				    size = n,
				    group = gatheringsinal,
				    color = gatheringsinal))
...
df2 <- filter(df, !is.na(gatheringsinal) & (!is.na(heightinal) | !is.na(widthinal))) %>%
       filter(gatheringsinal %in% top.gatherings) %>%
       group_by(gatheringsinal) %>% 
       summarize(
    	    mean.width = mean(widthinal, na.rm = T), 
	    median.width = mean(widthinal, na.rm = T),        
            mean.height = mean(heightinal, na.rm = T),
	    median.height = mean(heightinal, na.rm = T),
	    n = n())
