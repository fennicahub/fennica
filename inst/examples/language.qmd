...
`r sum(!df$multilingual & df$language_count > 0, na.rm = TRUE)` single-language entries (`r round(100 * mean(!df$multilingual & df$language_count > 0, na.omit = TRUE), 2)`%)

`r sum(df$multilingual, na.rm = TRUE)` multilingual entries , accounting for `r round(100 * mean(df$multilingual, na.omit = TRUE), 2)`% of the total. This includes entries explicitly coded as "mul" (Multiple languages) as well as those with more than one language listed for a single book.

There are `r sum(df$language_primary == "Undetermined")` entries marked as undetermined, coded as "und" in the dataset, accounting for (`r round(100*mean(df$language_primary == "Undetermined"), 2)`%) of the total. 
 
There are `r sum(is.na(df$languages))` missing values in the dataset. 

[Unrecognized language](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/language_discarded.csv) provides details of languages that were discarded. Additionally, the [Error list](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/language_discarded_id.csv) contains ID numbers of entries associated with these discarded languages, intended for librarian review.
...
`r sum(!df_19$multilingual & df_19$language_count > 0, na.rm = TRUE)` single-language entries (`r round(100 * mean(!df_19$multilingual & df_19$language_count > 0, na.omit = TRUE), 2)`%)

`r sum(df_19$multilingual, na.rm = TRUE)` multilingual entries (`r round(100 * mean(df_19$multilingual, na.omit = TRUE), 2)`%).

There are `r sum(df_19$language_primary == "Undetermined")` entries marked as undetermined.
