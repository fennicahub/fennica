...
Some dimension info is provided in the original raw data for altogether `r sum(!is.na(df$physical_dimensions))` documents (`r round(100*mean(!is.na(df$physical_dimensions)),1)`%) but could not be interpreted for `r sum(!is.na(df$physical_dimensions) & (is.na(df$gatherings)))` documents (ie. dimension info was successfully estimated for `r round(100 - 100 * sum(!is.na(df$physical_dimensions) & (is.na(df$gatherings)))/sum(!is.na(df$physical_dimensions)), 1)` % of th
