...
df_pubtime$publication_year_num <- as.numeric(df_pubtime$publication_year)

pre1488 <- df_pubtime |>
  filter(!is.na(publication_year_num)) |>
  filter(publication_year_num < 1488) |>
  select(id, publication_year_num)
...
n_before_1488 <- nrow(pre1488)

n_year_zero <- sum(pre1488$publication_year_num == 0, na.rm = TRUE)
```
...
Although Fennica is officially described as starting from 1488, the harmonized data contains `r n_before_1488` records of records with an available publication year) with publication years earlier than 1488. Of these, `r n_year_zero` records have publication year 0. These records were checked against the original metadata by system ID, including title and author name. 
...
pre1488 |>
  select(
    `System ID` = id,
    `Publication year` = publication_year_num,
    Title = title,
    Author = author_name
  ) |>
  kable(
    caption = "Records with publication years earlier than 1488"
  )
```
