...
-   [Unique accepted entries in original data](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/author_name_accepted_700.csv): `r length(unique(df_700[[field]]))`
...
-   Original documents with non-NA titles `r sum(!is.na(df_700[[field]]))` / `r nrow(df_700)` (`r round(100*mean(!is.na(df_700[[field]])), 1)`%)
-   Original documents with missing (NA) titles `r sum(is.na(df_700[[field]]))` / `r nrow(df_700)` documents (`r round(100*mean(is.na(df_700[[field]])), 1)`%)
...
theme_set(theme_bw(base_size = 12))
p <- top_plot(df_700, "author_name", ntop = ntop, log10 = TRUE)+
  labs(y = "Entries (n)", title = "Top titles (Log values)")
print(p)
```
...
-   [Unique accepted entries for for 1809-1917 years](dataTable/data_table.html?path=https://a3s.fi/swift/v1/AUTH_3c0ccb602fa24298a6fe3ae224ca022f/fennica-container/output.tables/author_name_accepted_700_19.csv): `r length(unique(df_700_19[[field]]))`
...
theme_set(theme_bw(base_size = 12))
p <- top_plot(df_700_19, "author_name", ntop = ntop, log10 = TRUE)+
  labs(y = "Entries (n)", title = "Top titles (Log values)")
print(p)
```
