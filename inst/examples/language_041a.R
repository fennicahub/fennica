...
for (myfield in c("language", "language_primary")) {
  tmp <- write_xtable(
    df.tmp[[myfield]],
    paste(output.folder, myfield, "_041_accepted.csv", sep = ""),
    count = TRUE,
...
tmp1 <- write_xtable(
  tab,
  paste(output.folder, field, "_041_conversions.csv", sep = ""),
  count = TRUE,
  add.percentages = TRUE
)
...
for (myfield in c("language", "language_primary")) {
  tmp <- write_xtable(df_19[[myfield]], paste(output.folder, myfield, "_041_accepted_19.csv", sep = ""), 
                      count = TRUE, 
                      add.percentages = TRUE)
}
...
# Convert to CSV and store in the output.tables folder
write.table(df_19, file = paste0(output.folder, paste0(field, "_041_19", ".csv")))
