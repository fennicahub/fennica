...
# 
# df_700_unique_rows <- df_700_by_record %>%
#   distinct()

df_700_by_unique_record <- df_700 %>%
  group_by(id1) %>%
  summarise(
    author_name_700 = paste(
      ifelse(is.na(author_name), "", author_name),
      collapse = "; "
    ),
    asteri_id_700 = paste(
      ifelse(is.na(asteri_id), "", asteri_id),
      collapse = "; "
    ),
    birth_700 = paste(
      ifelse(is.na(from), "", from),
      collapse = "; "
    ),
    death_700 = paste(
      ifelse(is.na(till), "", till),
      collapse = "; "
    ),
    .groups = "drop"
  )

df_700_by_unique_record <- df_700_by_unique_record %>%
  rename(id = id1)
