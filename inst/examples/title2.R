...
x <- polish_title_remainder(df.orig[[field]])

# Collect the results into a data.frame
df.tmp1 <- data.frame(melinda_id = df.orig$melinda_id,
                     title_remainder = x)

df.tmp$title <-  paste(df.tmp$title, df.tmp1$title_remainder, sep = "- ")

df.tmp$title <- gsub("- NA", "",df.tmp$title)

field <- "title"
# Define output files
file_accepted  <- paste0(output.folder, field, "2_accepted.csv")
file_discarded <- paste0(output.folder, field, "2_discarded.csv")
file_discarded_id <- paste0(output.folder, field, "2_discarded_id.csv")
